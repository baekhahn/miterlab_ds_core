...
const parseSpec = (rawSpec) => {
    return load(rawSpec);
};
const ensureSemanticToken = (token) => {
    if (!token.startsWith("semantic.")) {
        throw new Error(`Only semantic tokens are allowed in component mapping. Received: ${token}`);
    }
};
const getSemanticMapping = (spec) => spec.semanticMapping ?? {};
const pickVariantKey = (input, spec) => {
    ...
    if (requested === "neutral" && semanticMapping.secondary)
        return "secondary";
    if (requested === "critical" && semanticMapping.danger)
        return "danger";
    return requested;
};
const pickStateKey = (input, spec) => {
    return input.state ?? spec.states?.[0] ?? "default";
};
const pickToneKey = (input, spec) => {
    return input.tone ?? spec.tones?.[0];
};
const resolveButtonVariantKey = (input, spec) => {
    const semanticMapping = getSemanticMapping(spec);
    const fill = input.fill ?? "solid";
    const color = input.color ?? "default";
    const combined = `${fill}.${color}`;
    if (semanticMapping[combined])
        return combined;
    if (semanticMapping[`${fill}.default`])
        return `${fill}.default`;
    return combined;
};
const resolveButtonStateKey = (input) => {
    if (input.disabled || input.state === "disabled")
        return "disabled";
    if (input.loading === true || input.loading === "auto" || input.state === "loading")
        return "loading";
    if (input.state === "pressed" || input.state === "active")
        return "pressed";
    return input.state === "enabled" ? "enabled" : "default";
};
const resolveInputStateKey = (input) => {
    if (input.disabled || input.state === "disabled")
        return "disabled";
    if (input.readOnly || input.state === "readOnly" || input.state === "readonly")
        return "readonly";
    if (input.state === "focus" || input.state === "focused")
        return "focused";
    if (input.state === "loading")
        return "loading";
    return "enabled";
};
const isActionLike = (component) => [
    "Button",
    "IconButton",
    "TextButton",
    "Chip",
...
const resolveTextMetrics = (component, size) => {
    if (component === "Button") {
        if (size === "large")
            return { fontSize: 18, lineHeight: 25, fontWeight: "medium" };
        if (size === "small")
            return { fontSize: 15, lineHeight: 21, fontWeight: "medium" };
        if (size === "mini")
            return { fontSize: 13, lineHeight: 18, fontWeight: "medium" };
...
export const mapComponent = (input) => {
    const spec = parseSpec(input.rawSpec);
    const semanticMapping = getSemanticMapping(spec);
    const variantKey = spec.component === "Button"
        ? resolveButtonVariantKey(input, spec)
        : pickVariantKey(input, spec);
    const stateKey = spec.component === "Button"
    ...
    const buttonRenderSize = resolvedSize === "sm" || resolvedSize === "md" || resolvedSize === "lg" ? getButtonMetrics(resolvedSize) : undefined;
    const inputRenderSize = resolvedSize === "sm" || resolvedSize === "md" || resolvedSize === "lg" ? getInputMetrics(resolvedSize) : undefined;
    ...
    const specRadius = spec.component === "Button"
        ? shapeKey === "rounded"
            ? 999
    ...
    const defaultHeight = isCoreButton
        ? resolvedSize === "sm"
    ...
    const finalWidth = input.iconOnly && buttonLike
    ...
    const textValue = spec.component === "Input"
                            ...
                            : spec.component === "NavBar"
                                ...
                                : spec.component === "TabBar"
                                    ...
                                    : spec.component === "Form"
                                        ...
                                        : input.label;
    const variantPayload = spec.component === "Tabs"
        ? {
