...
import type { McpCapabilitySummary, McpToolCapability, McpToolCategory } from "./types/capabilities";

const inferCategory = (name: string): McpToolCategory => {
  if (name.includes("design_context") || name.includes("context")) return "context";
  ...
  if (name.includes("create") || name.includes("generate") || name.includes("insert") || name.includes("add")) return "create";
  if (name.includes("update") || name.includes("set") || name.includes("delete") || name.includes("remove") || name.includes("apply")) return "update";
  return "unknown";
};
...
export const discoverCapabilities = (tools: McpToolDefinition[]): McpCapabilitySummary => {
  const capabilities: McpToolCapability[] = tools.map((tool) => {
    const category = inferCategory(tool.name);
    const readOnlyHint = Boolean(tool.annotations && (tool.annotations as Record<string, unknown>).readOnlyHint === true);
  ...
  const categories = {
    ...
    style: 0,
    unknown: 0
  } as Record<McpToolCategory, number>;
