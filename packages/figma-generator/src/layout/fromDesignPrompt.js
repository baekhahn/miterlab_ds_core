import { normalizePrompt, resolveComponentType, validatePromptGrammar } from "../grammar/designPromptGrammar";
...
const toTitle = (screen) => {
    const clean = screen.replace(/[-_]/g, " ");
    return clean.charAt(0).toUpperCase() + clean.slice(1);
};
const toNodeName = (value, fallback) => {
...
const genericComponentTypes = new Set([
    "textarea",
    "select",
    "check-mark",
    "checkbox",
...
const resolveCoreButtonSize = (size) => {
    if (size === "sm" || size === "lg")
        return size;
    return "md";
};
const mapComponentToLayoutNode = (index, component, screen) => {
    ...
    if (componentType === "input") {
        const inputSize = component.size === "sm" || component.size === "md" || component.size === "lg"
            ? component.size
            : "md";
        ...
        return {
            ...
            component: "input",
            props: {
                intent: component.intent === "error" || component.intent === "success" || component.intent === "default"
                    ? component.intent
                    : "default",
                width: widthMode,
                size: inputSize,
                state: component.state === "focused" || component.state === "focus"
                    ? "focused"
    ...
    if (genericComponentTypes.has(componentType)) {
        ...
        const compactCatalogComponent = isInspectionScreen && (componentType === "tabs" ||
            componentType === "list" ||
    ...
    const inspectionButtonWidth = component.width === "full"
        ? getButtonWidth("full")
        : component.iconOnly
    ...
    return {
        ...
        component: "button",
        props: {
            appearance: component.appearance === "outlined" || component.appearance === "text"
                ? component.appearance
                : "solid",
            hierarchy: component.hierarchy === "primary-level-3" ||
                component.hierarchy === "assistive-level-2" ||
                component.hierarchy === "assistive-level-1" ||
                component.hierarchy === "destructive"
                ? component.hierarchy
                : "primary-level-4",
            emphasis: component.emphasis === "primary" ||
        ...
        width: component.width === "full" || component.block || component.fullWidth
            ? getButtonWidth("full")
            : isInspectionScreen
...
const estimateNodeHeight = (node) => {
    if (typeof node.height === "number")
        return node.height;
    if (node.type === "text")
        return layoutRules.textHeights.body;
    if (node.type === "component")
        return layoutRules.controlHeights.md;
    return 40;
};
const estimateSectionHeight = (node) => {
    if (node.type !== "stack") {
...
const createCatalogSectionChildren = (section, entries, sectionWidth, componentGap) => {
    if (entries.length === 0)
        return [];
    const children = [];
    let currentRow = [];
...
export const fromDesignPrompt = (prompt) => {
    ...
    const isCoreInspectionScreen = normalized.screen === "button-inspection" || normalized.screen === "input-inspection";
    const sectionSet = isCoreInspectionScreen
        ? new Set(normalized.sections ?? ["preview"])
        : new Set([
            ...(normalized.sections ?? []),
            ...pattern.requiredSections,
            ...pattern.optionalSections.filter((section) => normalized.components.some((component) => resolveSectionForComponent(component).section === section))
        ]);
    ...
    const isInspectionScreen = normalized.screen === "button-inspection" ||
    ...
    for (const section of orderedSections) {
        ...
        const sectionContentHeight = Math.max(44, sectionNode.children.reduce((sum, child) => sum + estimateNodeHeight(child), 0) +
            Math.max(0, sectionNode.children.length - 1) *
                (section === "header"
                    ? layoutRules.headerSpacing[normalized.density]
                    : section === "form"
                        ? formGap
    ...
    const contentBottom = children.length > 0
        ? Math.max(...children.map((child) => (child.y ?? 0) + estimateSectionHeight(child)), frameHeightBase) + layoutRules.framePadding.bottom
        : frameHeightBase;
    return {
        type: "frame",
